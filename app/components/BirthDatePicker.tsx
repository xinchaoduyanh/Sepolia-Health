    ...
    <>
      <TouchableOpacity
        ...
        className={`flex-row items-center rounded-lg px-4 py-4 ${
          error ? 'border border-red-200 bg-red-50' : 'bg-gray-100'
        }`}>
        <Ionicons name="calendar-outline" size={20} color={error ? '#EF4444' : '#000000'} />
        <View className="ml-3 flex-1">
          <Text className="text-base text-gray-800">
            {selectedDate ? formatDate(selectedDate) : placeholder}
          </Text>
          {selectedDate && (
            <Text className="text-sm text-gray-500">{getAge(selectedDate)} tuổi</Text>
          )}
        </View>
        ...
        <View className="flex-1 justify-end bg-black/50">
          <View className="rounded-t-3xl bg-white">
            <View className="flex-row items-center justify-between border-b border-gray-200 p-4">
              <Text className="text-lg font-semibold text-black">Chọn ngày sinh</Text>
              <View className="flex-row space-x-2">
                <TouchableOpacity onPress={handleCancel}>
                  <Text className="text-base text-gray-500">Hủy</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => handleDateSelect(tempDate)}>
                  <Text className="text-base font-semibold text-blue-500">Xong</Text>
                </TouchableOpacity>
              </View>
            </View>
            ...
            <View className="p-4">
              <View className="flex-row space-x-4">
                ...
                <View className="flex-1">
                  <Text className="mb-2 text-sm font-medium text-gray-600">Ngày</Text>
                    ...
                    {days.map((day) => (
                      <TouchableOpacity
                        ...
                        className={`rounded-lg p-2 ${
                          tempDate.getDate() === day ? 'bg-blue-100' : 'bg-gray-50'
                        }`}>
                        <Text
                ...
                {/* Month Picker */}
                <View className="flex-1">
                  <Text className="mb-2 text-sm font-medium text-gray-600">Tháng</Text>
                  <ScrollView className="h-32" showsVerticalScrollIndicator={false}>
                    {months.map((month) => (
                      <TouchableOpacity
                        key={month.value}
                        ...
                        className={`rounded-lg p-2 ${
                          tempDate.getMonth() + 1 === month.value ? 'bg-blue-100' : 'bg-gray-50'
                        }`}>
                        <Text
                          className={`text-center ${
                ...
                {/* Year Picker */}
                <View className="flex-1">
                  <Text className="mb-2 text-sm font-medium text-gray-600">Năm</Text>
                  <ScrollView className="h-32" showsVerticalScrollIndicator={false}>
                    {years.map((year) => (
                      <TouchableOpacity
                        key={year}
                        ...
                        className={`rounded-lg p-2 ${
                          tempDate.getFullYear() === year ? 'bg-blue-100' : 'bg-gray-50'
                        }`}>
                        <Text
                          className={`text-center ${
              ...
              {tempDate && (
                <View className="mt-4 rounded-lg bg-gray-50 p-3">
                  ...
                  <Text className="text-center text-sm text-gray-500">{getAge(tempDate)} tuổi</Text>
                </View>
              )}
            </View>
          </View>
