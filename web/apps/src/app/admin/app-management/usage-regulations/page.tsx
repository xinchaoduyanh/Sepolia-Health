...
export default function UsageRegulationsPage() {
    const [title, setTitle] = useState('')
    const [content, setContent] = useState('')
    const [isEditing, setIsEditing] = useState(false)

    const { data: activeTerms, isLoading, refetch } = useAppTermsByType(AppTermsType.USAGE_REGULATIONS)
    const createMutation = useCreateAppTerms()
    const updateMutation = useUpdateAppTerms()
    const activateMutation = useActivateAppTerms()

    // Update form when activeTerms changes
    useEffect(() => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold">Quy định sử dụng</h1>
                    <p className="text-muted-foreground mt-2">Quản lý quy định sử dụng ứng dụng Sepolia Health</p>
                </div>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>Nội dung quy định</CardTitle>
                    <CardDescription>
                        {activeTerms && (
                            <>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <TextareaField
                            ...
                            disabled={!isEditing}
                            placeholder="Nhập nội dung quy định (HTML hoặc markdown)"
