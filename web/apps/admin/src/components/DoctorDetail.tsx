...
import { ArrowLeft, Edit, Download, Share, MoreVertical, Calendar, Clock, MapPin, Phone, Mail, User, Building } from 'lucide-react'
...
export function DoctorDetail({ doctorId, onBack, onEdit }: DoctorDetailProps) {
    const [activeTab, setActiveTab] = useState('overview')
    const doctor = mockDoctorDetail // In real app, fetch by doctorId
            ...
            <Card className="p-6">
                <div className="flex items-start space-x-6">
                    <Avatar 
                        src={doctor.avatar} 
                        alt={doctor.fullName}
                        fallback={doctor.fullName.charAt(0)}
                        className="w-24 h-24"
                    />
                    ...
                    {doctor.recentAppointments.map((appointment) => (
                        <div key={appointment.id} className="flex items-center justify-between p-4 border border-border rounded-lg">
                            <div>
                                ...
                                <div className="text-sm text-slate-500">{appointment.date} - {appointment.time}</div>
                            </div>
